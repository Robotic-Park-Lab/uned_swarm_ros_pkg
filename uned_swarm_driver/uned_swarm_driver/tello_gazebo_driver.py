...
class TelloDriver(Node):
    def __init__(self):
        ...
        self.declare_parameter('robot', 'dron01')
        self.declare_parameter('type', 'virtual')

        # Subscription
        self.gt_pose_ = self.create_subscription(Odometry, 'odom', self.gtpose_callback, 10)
    ...
    def initialize(self):
        ...
        self.z_controller = PIDController(0.5, 0.0, 0.0, 0.0, 100, 1.0, -1.0, 0.1, 0.01)
        self.x_controller = PIDController(0.25, 0.0, 0.0, 0.0, 100, 1.0, -1.0, 0.1, 0.01)
        self.y_controller = PIDController(0.25, 0.0, 0.0, 0.0, 100, 1.0, -1.0, 0.1, 0.01)
    ...
    def take_off(self):
        ...
        # self.get_logger().info( 'Response %d' % response.rc)
        self.target_pose.position.z = 1.0
        self._is_flying = True

    def land(self):
        self.target_pose.position.z = 0.0
        self.get_logger().info('Descent...')
    ...
    def iterate(self):
        ...
        ## Position Controller
        # Z Controller
        if self.z_controller.eval_threshold(self.target_pose.position.z - self.gt_pose.position.z) or self.continuous:
            self.z_controller.error[0] = (self.target_pose.position.z - self.gt_pose.position.z)
            self.target_twist.linear.z = self.z_controller.update(0.1)
        else:
            self.target_twist.linear.z = self.z_controller.last_value

        # X-Y Controller
        # X Controller
        if self.x_controller.eval_threshold(self.target_pose.position.x - self.gt_pose.position.x) or self.continuous:
            self.x_controller.error[0] = self.target_pose.position.x - self.gt_pose.position.x
            self.target_twist.linear.x = self.x_controller.update(0.1)
        else:
            self.target_twist.linear.x = self.x_controller.last_value
        # Y Controller
        if self.y_controller.eval_threshold(self.target_pose.position.y - self.gt_pose.position.y) or self.continuous:
            self.y_controller.error[0] = self.target_pose.position.y - self.gt_pose.position.y
            self.target_twist.linear.y = self.y_controller.update(0.1)
        else:

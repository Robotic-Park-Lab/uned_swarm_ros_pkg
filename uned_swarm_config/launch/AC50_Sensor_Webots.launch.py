...
def generate_launch_description():
    ...
    config_path = os.path.join(general_package_dir, 'resources', 'AC50_RoboticPark.yaml')
    rviz_config_path = os.path.join(general_package_dir, 'rviz', 'AC50_RoboticPark.rviz')
    use_sim_time = LaunchConfiguration('use_sim_time', default=True)

    dron_package_dir = get_package_share_directory('uned_crazyflie_webots')
    webots = WebotsLauncher(
        world=os.path.join(general_package_dir, 'worlds', 'AC50_RoboticPark.wbt')
    )

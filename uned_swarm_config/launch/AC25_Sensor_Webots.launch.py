...
def generate_launch_description():
    ...
    config_path = os.path.join(general_package_dir, 'resources', 'AC25_RoboticPark.yaml')
    rviz_config_path = os.path.join(general_package_dir, 'rviz', 'AC25_RoboticPark.rviz')
    use_sim_time = LaunchConfiguration('use_sim_time', default=True)

    dron_package_dir = get_package_share_directory('uned_crazyflie_webots')
    webots = WebotsLauncher(
        world=os.path.join(general_package_dir, 'worlds', 'AC25_RoboticPark.wbt')
    )

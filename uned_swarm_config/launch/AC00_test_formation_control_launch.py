...
def generate_launch_description():
    ...
    config_path = os.path.join(config_package_dir, 'resources', 'AC30_RoboticPark.yaml')
    rviz_config_path = os.path.join(config_package_dir, 'rviz', 'AC30_RoboticPark.rviz')
    use_sim_time = LaunchConfiguration('use_sim_time', default=True)
    webots = WebotsLauncher(
        world=os.path.join(config_package_dir, 'worlds', 'AC30_RoboticPark.wbt')
    )
    ...
    with open(config_path) as f:
        ...
        for key, robot in data.items():
            ...
            if robot['description'] == 'crazyflie':
                robot_description = pathlib.Path(os.path.join(config_package_dir, 'resources', 'crazyflie.urdf')).read_text()
            else:
